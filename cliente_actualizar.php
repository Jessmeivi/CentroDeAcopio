...
<body>
    ...
    <?php
        if($_POST)//Recibe los datos del formulario de abajo
        {
            ...
            if(!$conexion){
                ...
                echo "<br>";
                $sql ="call listadoC($id)"; //Con esta consulta filtra todas las preguntas que contengan esa palabra clave
                $result = $conexion->query($sql);
                if($result->num_rows > 0){
                    echo "<center><div style='width:50%'>";
                    echo "<center><div><h1 style='color:  #FFFFFF; '>ACTUALIZAR CLIENTE</h1></div></center>";
                    echo "<form name='elim' action='http://localhost/conexion/actu_cliente.php' method='POST'>" ;
                    echo "<table>";
                    while($row = $result->fetch_assoc()){
                        echo"<div class='input-container' > <input type='text'  style='color:#FFFFFF' name='nombre' value='".$row["v1"]."'> <label style='color: #FFFFFF; '>Nombre: </label> </div><br>";
                
                        echo"<div class='input-container'><input type='text'  style='color:#FFFFFF' name='ape_pat' value='".$row["v2"]."'><label style='color: #FFFFFF; '>Apellido Paterno: </label> </div><br>";
                
                        echo"<div class='input-container'><input type='text'  style='color:#FFFFFF' name='ape_mat' value='".$row["v3"]."'><label style='color: #FFFFFF; '>Apellido Materno: </label> </div><br>";
                
                        echo "<div class='input-container'><input type='text'  style='color:#FFFFFF' name='estado' value='".$row["v4"]."'><label style='color: #FFFFFF; '>Estado: </label> </div><br>";
                        echo "<div class='input-container'><input type='text' style='color:#FFFFFF' name='colonia' value='".$row["v5"]."'><label style='color: #FFFFFF; '>Colonia: </label> </div><br>";
                 
                        echo"<div class='input-container'><input type='text' style='color:#FFFFFF' name='calle' value='".$row["v6"]."'><label style='color: #FFFFFF; '>Calle: </label> </div><br>";
                 
                        echo"<div class='input-container'><input type='number' style='color:#FFFFFF' name='numero' value='".$row["v7"]."'><label style='color: #FFFFFF; '># Casa: </label> </div><br>";

                        echo"<div class='input-container'><input type='number' style='color:#FFFFFF' name='telefono' value='".$row["v8"]."'><label style='color: #FFFFFF; '>Teléfono</label> </div><br>";
                        echo"  <tr>";
                        echo"<td>";
    ...
    ?>
            <table class="center">
                <tr>
                    <td align="center">
                        <h1>Actualizar cliente por id </h1> <!--Formulario para crear al nuevo usuario--> 
                    </td>
                </tr>
                <tr>
                    <td align="center">
                        <form name="Registro" action="http://localhost/conexion/cliente_actualizar.php" method="POST"> <!--Igualmente el formulario se envia a la misma pagina--> 
                            <div class="input-container">
                                <input type="numeric" style="color:#FFFFFF" name="id"><label>ID</label><br>
                            </div>
                            <table>
                                <tr>
                                    <td>
